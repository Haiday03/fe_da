...
export class NewsComponent { 
    // search variable
    searchNewsDto = new SearchNewDto({ keyword: "", authorId: undefined, fromDate: "", toDate: "" });
    ...
    constructor(private newService: NewService, private authorService: AuthorService, private messageService: MessageService,private bookService: BookService) { }
    ...
    fetchPaging(){
        this.newService.getPage(this.pagination, this.searchNewsDto).subscribe(
            data => {
    ...
    _checkRequire(){
        if(!this.new.title || !this.new.author || !this.new.releaseDate || !this.new.keyword || !this.new.summary || !this.new.content){
            return false;
        }
        return true;
    }
    ...
    edit(id){
        ...
        this.dialogAddOrUpdate = true;
        this.refreshObject();
        this.newService.getOne(id).subscribe(
            res => {
                this.new = res;
