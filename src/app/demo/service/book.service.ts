...
export class BookService {
  ...
  getAll(myparams = {}): Observable<any> {
    ...
    return this.apiService.getAllWithParams(this.BOOK_PATH, myparams ? myparams : {}).pipe(map(
      res => {
        if (res) {
          return res;
        } else {
  ...
  uploadImage(formData): Observable<any> {
    return this.apiService.post(this.UPLOAD_IMAGE, formData).pipe(map(
      // res => {
      //   if (res) {
      //     return res;
      //   } else {
  ...
  upload(file : File):Observable<any> { 
    ...
    return this.http.post("http://localhost:9091/api/book/upload/image", formData, { observe: 'response', responseType: 'text' });
  } 
  ...
  getPage(pagination: PaginationDto, searchBookDto: SearchBookDto){
    ...
    return this.apiService.getAll(`${this.BOOK_PATH}/get/page?page=${pagination.page}&size=${pagination.size}${conditionStr}`).pipe(map(
      res => {
        if (res) {
          return res;
        } else {
